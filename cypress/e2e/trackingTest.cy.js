...
describe('track order', () => {
  ...
  it('should track two correct orders', () => {
    ...
    cy.get('[data-cy=trackingCode_input]').eq(0).clear();
    cy.get('[data-cy=trackingCode_input]').eq(0).type(exampleTrackingInfo[1].trackingCode);
    cy.get('[data-cy=extraVerification_input]').eq(0).clear();
    cy.get('[data-cy=extraVerification_input]').eq(0).type(exampleTrackingInfo[1].verificationCode);
    cy.get('[data-cy=track_button]').eq(0).click();
    // cy.wait(500);
    cy.get('[data-cy=status_steps]').should('be.visible');
  });
